...
export default function RewardsQueue({ pending, recent }: Props) {
  ...
  async function act(id: number, action: 'fulfil' | 'dismiss') {
    ...
    setActing(null)
    if (res.ok) {
      setItems((prev) => prev.filter((threshold) => threshold.id !== id))
  ...
  return (
    <div className="space-y-6">
      <section>
        {items.length === 0 ? (
          ...
          <div className="space-y-3">
            {items.map((threshold) => (
              <div key={threshold.id} className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <span className="text-xs font-medium text-amber-600 uppercase tracking-wide">
                      {TRACK_LABELS[threshold.track] ?? threshold.track}
                    </span>
                    <p className="font-semibold mt-0.5">{threshold.reward_description}</p>
                    <p className="text-xs text-gray-500 mt-0.5">
                      {threshold.points_required} pts threshold / Requested {new Date(threshold.requested_at!).toLocaleDateString('en', {
                        day: 'numeric',
                        month: 'short',
                        hour: '2-digit',
                        minute: '2-digit',
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => act(threshold.id, 'fulfil')}
                    disabled={acting === threshold.id}
                  ...
                  >
                    {acting === threshold.id ? 'Saving...' : 'Mark fulfilled'}
                  </button>
                  <button
                    onClick={() => act(threshold.id, 'dismiss')}
                    disabled={acting === threshold.id}
      ...
      {recentItems.length > 0 && (
        <section>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Recent history</h3>
          <div className="space-y-2">
            {recentItems.map((threshold) => (
              <div key={threshold.id} className="bg-white rounded-xl shadow-sm p-3 flex items-center gap-3">
                <span className="text-sm">{TRACK_LABELS[threshold.track] ?? threshold.track}</span>
                <span className="text-sm text-gray-700 flex-1">{threshold.reward_description}</span>
                <span className={`text-xs font-medium shrink-0 ${threshold.fulfilled_at ? 'text-green-600' : 'text-gray-400'}`}>
                  {threshold.fulfilled_at ? 'Fulfilled' : 'Dismissed'}
                </span>
              </div>
            ))}
          </div>
