...
export async function GET() {
  ...
  const [settings] = await sql`SELECT current_level FROM track_settings WHERE track = 'french' AND child_user_id = ${userId}`
  const level = settings?.current_level ?? 5

  try {
    const aiModel = await getConfiguredAiModel()
    const text = await generateText(withReadingTopic(frenchReadingPrompt(level)), aiModel)
...
export async function POST(req: NextRequest) {
  ...
  const [settings] = await sql`SELECT points_per_entry FROM track_settings WHERE track = 'french' AND child_user_id = ${userId}`
  const points = settings?.points_per_entry ?? 10

  const [entry] = await sql`
    INSERT INTO entries_french (user_id, date, ai_generated_text, level_at_time, done, points_awarded, audio_key)
...
export async function PATCH(req: NextRequest) {
  ...
  const userId = parseInt(session.user.id)
  const [entry] = await sql`
    UPDATE entries_french SET audio_key = ${audio_key ?? null}
